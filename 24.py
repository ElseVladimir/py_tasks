...
#     <var> при запросе из пространства <namespace>, или None, если такого пространства не существует
#
...
# пространства global. В нашем случае, это описывается командой create foo global. Далее мы объявляем
# внутри функции foo функцию bar, тем самым создавая пространство bar внутри пространства foo, и добавляем
# в bar переменную a.
#
...
while True:
    ...
    def create():
        global namespaces
        namespaces.update({command[1]: {'variables': [], 'parent': command[2]}})
        #print(namespaces)
    def get():
        var = ''
        for a, b in reversed(namespaces.items()):
            if command[2] in b['variables']:
                var += a
                print(var)
                break
        if not var:
